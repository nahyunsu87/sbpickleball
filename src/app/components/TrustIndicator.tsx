...
function computeBadges(snapshot: TrustSnapshot): TrustBadgeId[] {
  ...
  if (reliability.isMeasured && activity.totalGames > 0 && reliability.noShowCount6m === 0) {
    badges.push('no_noshow')
  }
...
async function fetchTrustSnapshot(userId: string): Promise<TrustSnapshot> {
  ...
  const activeTeamsMap: Record<string, boolean> = {}
  ;(participants || []).forEach((p: any) => {
    if (p.matches?.status === 'active' && p.team) activeTeamsMap[p.team] = true
  })
  ...
  return {
    ...
    activity: {
      ...
      activeTeamCount: Object.keys(activeTeamsMap).length,
      lastMatchDate,
    },
    manner: {
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        ...
        <div className="bg-gray-50 rounded-xl p-3 text-center">
          <p className={`text-lg font-extrabold ${reliability.noShowCount6m === 0 ? 'text-emerald-600' : 'text-red-500'}`}>
            ✅ {reliability.isMeasured ? reliability.noShowCount6m : '—'}
          </p>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-white border border-gray-100 rounded-xl p-3">
          ...
          {reliability.isMeasured ? (
            ...
            <p className="text-xs text-gray-400">노쇼/취소/지각 데이터 집계 준비중</p>
          )}
        </div>
        <div className="bg-white border border-gray-100 rounded-xl p-3">
